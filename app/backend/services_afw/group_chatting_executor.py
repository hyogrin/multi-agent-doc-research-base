...
class GroupChattingExecutor(Executor):
    ...
    def __init__(
        ...
        logger.info(f"GroupChattingExecutor initialized with context_max_chars={context_max_chars}, max_document_length={max_document_length}")
    ...
    async def run_group_chat(
        ...
        try:
            ...
            logger.info(f"[GroupChattingExecutor] Starting group chat for {len(sub_topics)} sub-topics")
            ...
            if not sub_topics:
                logger.info("[GroupChattingExecutor] No sub_topics provided, creating default")
                sub_topics = [{
            ...
            for idx, sub_topic_data in enumerate(sub_topics, 1):
                ...
                logger.info(f"[GroupChattingExecutor] Sub-topic '{sub_topic_name}' has {len(context_parts)} context items ({len(context)} chars)")
                ...
                if status == "success":
                    ...
                    if not final_answer:
                        ...
                        logger.warning(f"[GroupChattingExecutor] No answer for '{sub_topic_name}'")
                    
                    ready_icon = "✅" if ready_to_publish else "⚠️"
                    
                    # Yield sub-topic header with score
                    await ctx.yield_output(f"\n")
                    await ctx.yield_output(f"data: ### {RESULT_MSG.get('write_research', 'Writing Answer')} for {sub_topic_name} {ready_icon} (Score: {reviewer_score})\n\n")
    ...
    async def _execute_single_sub_topic(
        ...
        try:
            ...
            try:
                ...
                answer_markdown = parsed_answer.get("revised_answer_markdown", "") or parsed_answer.get("draft_answer_markdown", "")
                ...
                logger.info(f"[GroupChattingExecutor] Parsed answer for '{sub_topic}': {len(answer_markdown)} chars, score={reviewer_score}, ready={ready_to_publish}")
                
            except json.JSONDecodeError as e:
                logger.warning(f"[GroupChattingExecutor] Failed to parse JSON for '{sub_topic}': {e}")
                logger.warning(f"[GroupChattingExecutor] Raw response: {final_answer[:200]}...")
                # Fallback: use raw response
                answer_markdown = final_answer
            except Exception as e:
                logger.error(f"[GroupChattingExecutor] Unexpected error parsing answer: {e}")
                answer_markdown = final_answer
