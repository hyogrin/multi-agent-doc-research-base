...
try:
    ...
    MCP_AVAILABLE = False
    logger.warning("MCP library not installed. YouTube MCP functionality will be disabled.")
...
class YouTubeMCPExecutor(Executor):
    ...
    def __init__(
        ...
        if not MCP_AVAILABLE:
            logger.warning("MCP library not available. YouTube search will be disabled.")
    ...
    async def _ensure_mcp_connection(self):
        ...
        if self.mcp_session is None and MCP_AVAILABLE and self.api_key:
            try:
                ...
                logger.info(f"YouTube MCP server connected. Available tools: {tool_names}")
                    ...
                    for content_item in search_result.content:
                        if hasattr(content_item, "text"):
                            ...
                            try:
                                ...
                                logger.warning(f"Could not parse JSON from result: {result_text[:100]}")
