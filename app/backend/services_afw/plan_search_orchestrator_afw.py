    ...
    async def analyze(
        ...
        try:
            ...
            try:
                ...
                logger.info(f"IntentAnalyzerExecutor: Intent = {intent_data.get('user_intent')}")
                logger.info(f"IntentAnalyzerExecutor: Enriched query = {intent_data.get('enriched_query')}")
                logger.info(f"IntentAnalyzerExecutor: Confidence = {intent_data.get('confidence')}")
                ...
                if verbose:
                    intent_data_str = json.dumps(intent_data, ensure_ascii=False, indent=2)
                    await ctx.yield_output(f"data: {send_step_with_code(LOCALE_MSG['analyze_complete'], intent_data_str)}\n\n")
                ...
                logger.error(f"IntentAnalyzerExecutor: Response text: {response_text[:500]}")
                ...
                await ctx.yield_output(f"data: {send_step_with_code(LOCALE_MSG['analyze_complete'], fallback_data)}\n\n")
            ...
            await ctx.send_message({
                "error": str(e),
                "user_intent": "general_query",
                "enriched_query": last_user_message if 'last_user_message' in locals() else "",
                "search_query": last_user_message if 'last_user_message' in locals() else "",
                "original_query": last_user_message if 'last_user_message' in locals() else "",
                "confidence": 0.5,
                "keywords": [],
                "target_info": "general information",
                "tool_name": "",
...
class TaskPlannerExecutor(Executor):
    ...
    async def plan(
        ...
        try:
            ...
            logger.info(f"Using {'RESEARCH' if user_intent == 'research' else 'GENERAL'} planner prompt")
            
            # Call LLM for planning
            af_messages = [
                AFChatMessage(
                    role=Role.SYSTEM,
                    contents=[TextContent(text="You are an expert search planner. Always respond with valid JSON.")]
                ),
                AFChatMessage(
                    role=Role.USER,
                    contents=[TextContent(text=planner_prompt)]
                )
            ]
            ...
            try:
                ...
                for item in search_queries_data:
                    if isinstance(item, dict) and 'sub_topic' in item and 'queries' in item:
                ...
                logger.info(f"SearchPlannerExecutor: Generated {len(sub_topics)} sub-topics with {len(flat_queries)} total queries")
                ...
                if verbose:
                    plan_data_str = json.dumps(plan_result, ensure_ascii=False, indent=2)
                    await ctx.yield_output(f"data: {send_step_with_code(LOCALE_MSG['plan_done'], plan_data_str)}\n\n")
                else:
                    await ctx.yield_output(f"data: {send_step_with_code(LOCALE_MSG['plan_done'], plan_data_str)}\n\n")
...
class ResponseGeneratorExecutor(Executor):
    ...
    async def generate(
        ...
        try:
            ...
            context_str = "\n\n".join(all_contexts) if all_contexts else "No additional context available."
            ...
            af_messages = [
                AFChatMessage(
                    role=Role.SYSTEM,
                    contents=[TextContent(text=response_prompt_text)]
                ),
                AFChatMessage(
                    role=Role.USER,
                    contents=[TextContent(text=enriched_query)]
                )
            ]
...
class PlanSearchOrchestratorAFW:
    ...
    def __init__(self, settings: Settings):
        ...
        logger.info(f"PlanSearchOrchestrator initialized with Azure OpenAI deployment: {settings.AZURE_OPENAI_DEPLOYMENT_NAME}")
        ...
        try:
            ...
            if os.getenv("BING_GROUNDING_PROJECT_ENDPOINT"):
                grounding_executor = GroundingExecutor(
                    ...
                    agent_model_deployment_name=os.getenv("BING_GROUNDING_AGENT_MODEL_DEPLOYMENT_NAME"),
                    max_results=5,
            ...
            if stream:
                ...
                for event in events:
                    ...
                    # Get final output
                    if isinstance(event, WorkflowOutputEvent):
                        final_output = event.output if hasattr(event, 'output') else event_data
                ...
                # Add elapsed time
                if elapsed_time:
                    elapsed = (datetime.now(tz=self.timezone) - start_time).total_seconds()
                    elapsed_msg = LOCALE_MSG.get('elapsed_time', 'Elapsed Time')
                    yield f"\ndata: ### {elapsed_msg}: {elapsed:.2f}s\n\n"
        except Exception as e:
            logger.error(f"Error in PlanSearchOrchestrator.generate_response: {str(e)}", exc_info=True)
