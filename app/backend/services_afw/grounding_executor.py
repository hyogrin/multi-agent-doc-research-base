...
class GroundingExecutor(Executor):
    ...
    def __init__(
        ...
        self.project_endpoint = project_endpoint or os.getenv("BING_GROUNDING_PROJECT_ENDPOINT")
        ...
        logger.info(f"GroundingExecutor initialized with agent: {self.search_gen_agent.id}")
    ...
    def _initialize_agent(self):
        ...
        try:
            if self.search_gen_agent_id_env:
                logger.info(f"Retrieving existing agent: {self.search_gen_agent_id_env}")
    ...
    async def grounding_search(
        ...
        try:
                ...
                try:
                    ...
                    if isinstance(queries_data, list):
                        ...
                        if queries_data and isinstance(queries_data[0], dict):
                            queries = [q.get("query", q.get("search_query", "")) for q in queries_data]
                        else:
            ...
            if verbose and result:
                ...
                await ctx.yield_output(f"data: {send_step_with_code(LOCALE_MSG['search_done'], encoded_code)}\n\n")
            else:
